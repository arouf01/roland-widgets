const getSornowert = (sornowertData, Name) => {
  ...
  if (filteredData) {
    ...
    filteredData.forEach((data) => {
      ...
      const Sornowert = data["Sornowert"] || 0.0;
      ...
      <td class="border px-4 py-1 break-words text-left align-middle">${Stornogrund}</td>
      <td class="border px-4 py-1 break-words text-right align-middle">${parseFloat(
        Sornowert || 0
      ).toFixed(2)}</td>
    `;
